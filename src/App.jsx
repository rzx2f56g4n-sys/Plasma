...
const MODES = [
  { id: "general", label: "General", icon: "⚡", color: "#e040fb", prompt: "You are Plasmed, a highly intelligent and helpful AI assistant for medical practices." },
  { id: "medical", label: "Medical", icon: "🏥", color: "#00c853", prompt: "You are Plasmed, an AI assistant for medical professionals. You help doctors and dentists with patient communication, clinical summaries, referral letters, medical reports, and answering common patient questions professionally and accurately. Always maintain patient confidentiality and recommend consulting a qualified physician for specific medical advice." },
  { id: "receptionist", label: "Receptionist", icon: "📋", color: "#00e5ff", prompt: "You are Plasmed in Receptionist mode. You help medical receptionists draft appointment confirmations, patient reminders, follow-up messages, and handle general patient enquiries in a warm, professional manner." },
  { id: "reports", label: "Reports", icon: "📝", color: "#69ff47", prompt: "You are Plasmed in Reports mode. You help medical professionals write and structure clinical notes, patient reports, referral letters, and medical certificates clearly and professionally." },
  ...
  if (!session) return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0a1f14, #0d2818)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "sans-serif" }}>
      <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, padding: 40, width: 360 }}>
        ...
        {authError && <div style={{ color: authError.includes("Check") ? "#00c853" : "#ff6d00", fontSize: 13, marginBottom: 12, textAlign: "center" }}>{authError}</div>}
        <button onClick={handleAuth} style={{ width: "100%", padding: "13px", borderRadius: 10, border: "none", background: "linear-gradient(135deg, #00c853, #00e5ff)", color: "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer", marginBottom: 16 }}>
          {authMode === "login" ? "Sign In" : "Create Account"}
        </button>
        <div style={{ textAlign: "center", color: "rgba(255,255,255,0.4)", fontSize: 13 }}>
          {authMode === "login" ? "No account? " : "Have an account? "}
          <span onClick={() => setAuthMode(authMode === "login" ? "signup" : "login")} style={{ color: "#00c853", cursor: "pointer" }}>
